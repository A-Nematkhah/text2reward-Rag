...
────────────────────
LEGACY MODULE — kept for backward compatibility with evaluate.py and
...
The weight-based compute_shaped_reward() is preserved so evaluate.py with
--no-shaped still works unchanged.

Migration guide
───────────────
  Replace:  from reward_components import compute_shaped_reward, load_weights
  With:     from reward_sandbox import execute_reward, build_state
            from reward_wrapper import _load_reward_fn
"""
...
# ── Minimal weight stubs (so training_logger import doesn't break) ────────────
...
def load_weights(path: str = WEIGHTS_FILE) -> dict[str, float]:
    ...
    if os.path.exists(path):
        try:
            ...
            print(f"[weights] Failed to load '{path}': {e} — using defaults")
