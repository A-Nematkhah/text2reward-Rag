...
def plot_generation_evolution(
    ...
    """
    if not llm_updates:
        log.info("  [plot] No LLM updates recorded — skipping generation_evolution.png")
        return
...
def plot_llm_impact(
    ...
    if not llm_updates:
        log.info("  [plot] No LLM updates recorded — skipping llm_impact.png")
        return
...
def plot_archive_fitness(
    ...
    if not os.path.exists(archive_path):
        log.info(f"  [plot] Archive not found: '{archive_path}' — skipping archive_fitness.png")
        return
    ...
    if not entries:
        log.info("  [plot] Archive has no entries — skipping archive_fitness.png")
        return
...
def generate_all_plots(
    ...
    if not os.path.exists(log_path):
        log.info(f"[plot] Log file not found: '{log_path}'")
        log.info("  Run train.py first to generate the log.")
        return

    with open(log_path, "r", encoding="utf-8") as f:
        training_log = json.load(f)

    episodes = training_log.get("per_episode", [])
    llm_updates = training_log.get("llm_updates", [])

    if not episodes:
        log.info("[plot] No episode data found in log — nothing to plot.")
        return

    os.makedirs(out_dir, exist_ok=True)

    log.info(f"[plot] {len(episodes)} episodes | {len(llm_updates)} generation switches")
    log.info(f"[plot] Output directory: '{out_dir}/'")
