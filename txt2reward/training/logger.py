"""
Persistent JSON training log for plots and resume.

Writes ``training_log.json`` (see ``config.paths.LOG_FILE``) with per-episode
records and one entry per completed LLM evolution (``llm_updates``).
"""
...
from txt2reward.config.paths import LOG_FILE
...
class TrainingLogger:
    """Append-only training telemetry with optional resume from disk."""

    def __init__(self, log_path: str = LOG_FILE):
        """Load prior log when ``log_path`` exists; otherwise start empty.

        Side effects:
            Reads JSON from ``log_path``; logs resume summary or warnings.
        """
        ...
        if os.path.exists(log_path):
            try:
                ...
                log.info(
                    "[logger] Resumed from '%s' (%s episodes, %s LLM updates)",
                    log_path,
                    len(self.per_episode),
                    len(self.llm_updates),
                )
            except Exception as e:
                log.warning("[logger] Could not load old log: %s — starting fresh", e)
    ...
    def log_episode(
        ...
        """Record one completed episode (called from evolution callback).

        Args:
            stats: ``info['episode_stats']`` from ``LLMRewardWrapper``.
            timestep: Global PPO timestep when the episode ended.
            weights: Active generation metadata (``generation``, ``reward_path``).
            policy_snap: Optional averaged PPO health metrics.

        Side effects:
            Appends to ``per_episode`` in memory only; call ``save()`` to persist.
        """
        self._episode_n += 1
    ...
    def log_llm_update(
        ...
        """Record a completed reward evolution (new generation on disk).

        Side effects:
            Appends to ``llm_updates`` in memory only.
        """
        record: dict[str, Any] = {
            "episode": episode,
            "timestep": timestep,
    ...
    def save(self) -> None:
        """Atomically write the full log to ``self.log_path``."""
        data = {
            "meta": {
    ...
    def save_periodically(self, every_n: int = 10) -> None:
        """Persist when ``episode_count()`` is a multiple of ``every_n``."""
